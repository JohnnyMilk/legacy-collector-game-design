...
export function mountCombatShell(root,{brandHref='../combat-test-index.html',demoLabel='Combat'}={}){
  root.innerHTML=`
    ...
    <div class="hud-layer">
      ...
      <section id="skill-hud" class="skill-hud glass-panel" hidden><div class="skill-hud-head"><span>主動技能</span><span id="skill-owner">—</span></div><div id="skill-list" class="skill-list"></div></section>
...
export class CombatApp{
  constructor({root,scenario,brandHref,demoLabel='Combat',aiDelay=420,progressRows=[],resultContent,portraitBase='../../assets/units'}){
    this.root=root;this.scenario=scenario;this.aiDelay=aiDelay;this.progressRows=progressRows;this.resultContent=resultContent;this.portraitBase=portraitBase;this.mode='idle';this.selectedUnitId=null;this.selectedSkillId=null;mountCombatShell(root,{brandHref,demoLabel});this.cacheElements();this.bindEvents();this.reset();
  }
  q(selector){return this.root.querySelector(selector)}
  cacheElements(){this.grid=this.q('#battle-grid');this.log=this.q('#combat-log');this.turn=this.q('#turn-info');this.detail=this.q('#unit-detail');this.result=this.q('#battle-result');this.timeline=this.q('#timeline');this.hud=this.q('.hud-layer');this.skillHud=this.q('#skill-hud');this.skillList=this.q('#skill-list');this.skillOwner=this.q('#skill-owner');this.partyPassiveContent=this.q('#party-passive-content')}
  reset(){this.model=new CombatModel(this.scenario);this.partyComposition=evaluatePartyComposition(this.model.units);this.model.partyComposition=this.partyComposition;this.mode='idle';this.selectedUnitId=null;this.selectedSkillId=null;this.model.start();this.result.hidden=true;this.skillHud.hidden=true;this.hud.classList.remove('minimal');this.root.querySelectorAll('.compact').forEach(p=>p.classList.remove('open'));this.renderProgress();this.render();this.driveAI()}
  ...
  renderDetail(){const selected=this.selectedUnitId?this.model.units.find(u=>u.id===this.selectedUnitId&&u.alive):null,u=selected||this.model.currentUnit();if(!u){this.detail.innerHTML='<span class="unit-note">點選場上單位查看基本狀態。</span>';return}this.q('#context-title').textContent=u.label;const passive=(u.passiveSkills||[])[0];this.detail.innerHTML=`<div class="detail-head"><strong>${u.label}</strong><span>${u.team==='player'?'職業 '+u.className:`${u.tierLabel||'Tier 1'} ${u.className}`}</span></div><div class="inspect-hp"><div class="inspect-hp-row"><span>生命狀態</span><span>${this.hpPct(u)>66?'穩定':this.hpPct(u)>33?'受傷':'危急'}</span></div><div class="inspect-hp-bar"><div class="inspect-hp-fill" style="width:${this.hpPct(u)}%"></div></div></div>${passive?`<div class="unit-note"><strong>被動｜${passive.name}</strong><br>${passive.effect}</div>`:'<div class="unit-note">戰鬥參數不公開；依實際交戰與戰況判斷威脅。</div>'}`}
