...
function randomTier2Enemies(enemyStats,count){const types=Object.keys(enemyStats?.tiers?.tier2?.entries||{});if(!types.length)throw new Error('NO_TIER2_ENEMIES');return Array.from({length:count},()=>randomPick(types))}
...
async function buildScenario(){
  const [raw,stats,classes,enemyStats]=await Promise.all([
    loadCombatJson('./scenarios/demo-2-2.json'),
...
try{
  ...
  const app=new Tier2BossCombatApp({root:document.querySelector('#game-screen'),scenario,progressRows,brandHref:'../combat-test-index.html',demoLabel:'Demo 2-2 / Region 2 Limit',resultContent:result,skillPageSize:1,passivePageSize:1});
  installFloatingCombatText(app);
}catch(error){console.error(error);document.querySelector('#game-screen').innerHTML='<div class="result-overlay"><div class="result-card"><h2>Demo 2-2 載入失敗</h2><p>請重新整理後再試。</p></div></div>'}
