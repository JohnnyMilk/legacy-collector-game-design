...
function onCell(x,y,u){
  ...
  if(current?.team==='player'&&mode==='attack'){
    ...
    if(legalTarget){
      selectedUnitId=null;previewTargetId=null;
  ...
  if(current?.team==='player'&&mode==='move'){
    if(!u&&model.move(current,x,y)){mode='idle';selectedUnitId=current.id;previewTargetId=null;render();return}
    return;
  }
...
function renderButtons(){const u=model.currentUnit(),player=u?.team==='player'&&!model.finished;$('#move-btn').disabled=!player||u.moved;$('#attack-btn').disabled=!player||u.acted||model.validTargets(u).length===0;$('#end-btn').disabled=!player;$('#end-btn').textContent=player&&!u.moved&&!u.acted?'等待':'結束';$('#move-btn').classList.toggle('active',mode==='move');$('#attack-btn').classList.toggle('active',mode==='attack')}
