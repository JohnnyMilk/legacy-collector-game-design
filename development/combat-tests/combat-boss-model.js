...
export class BossCombatModel extends CombatModel{
  ...
  dealDamage(attacker,target,options={}){const before=target.currentHP;const result=super.dealDamage(attacker,target,options);if(result?.hit&&target.alive&&target.currentHP<before)this.maybeCoreRegenerate(target);return result}
  applyHuntingInstinct(unit,movedBefore,result){if(result?.hit&&movedBefore&&this.hasPassive(unit,'hunting-instinct')){unit.tempEvaBonus=15;this.addLog(`${unit.label}「獵殺本能」啟動：EVA +15。`)}}
