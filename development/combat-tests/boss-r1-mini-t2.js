...
async function buildScenario(){
  ...
  return {...raw,id:'boss-r1-mini-t2',title:'Boss Test A-1｜Region 1 Mini Boss vs Tier 2',units:[...raw.players.map((p,i)=>buildTier2BenchmarkUnit(p,classIds[i],stats,classes)),buildBossUnit(raw.boss,'miniBoss',bossStats)]};
}
...
try{const scenario=await buildScenario();new BossCombatApp({root:document.querySelector('#game-screen'),scenario,brandHref:'../combat-test-index.html',demoLabel:'Boss A-1 / T2',resultContent:result})}catch(error){console.error(error);document.querySelector('#game-screen').innerHTML='<div class="result-overlay"><div class="result-card"><h2>Boss A-1 載入失敗</h2><p>請重新整理後再試。</p></div></div>'}
