...
try{const {scenario,progressRows}=await buildScenario();new Tier2BossCombatApp({root:document.querySelector('#game-screen'),scenario,progressRows,brandHref:'../combat-test-index.html',demoLabel:'Boss A-1 / T2',resultContent:result,passivePageSize:1})}catch(error){console.error(error);document.querySelector('#game-screen').innerHTML='<div class="result-overlay"><div class="result-card"><h2>Boss A-1 載入失敗</h2><p>請重新整理後再試。</p></div></div>'}
