...
- `CONFIRMED` 代表該職業技能與完整 Mastery Checklist 都已定案。
- 若職業技能已定案、但 Mastery 尚未完成，卡片顯示 `DRAFT · Mastery 待設計`，技能內容仍可查看，不再把已定案技能誤標成「待定義」。
- 完全尚未設計的內容才標示「待定義」，不使用猜測資料補滿。
- Mastery 的互動流程測試獨立放在 `development/mastery-flow-demo.html`，不嵌入每張職業卡。

目前 Tier 2「煉金術士」已完成一般攻擊、被動「再調製」與主動「藥劑投擲」設計，但 3 項職業專屬 Mastery 尚未建立，因此目前維持 draft。
