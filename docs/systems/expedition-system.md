...
每個 Region 通關後、正式進入下一個 Region 前，隊伍進行一次**完整恢復**。此規則是 Region 間的固定節奏，不等同於一般戰鬥結束後的恢復。

- 隊伍中已死亡的角色全部復活。
- 全體角色 HP 恢復至最大值。
- 所有目前可用主動技能的 Charge／使用次數恢復至上限。
- 完整恢復完成後，才開始下一個 Region。

因此死亡與資源消耗仍會影響單一 Region 內的戰鬥與路線壓力，但不會讓上一個 Region 的死亡角色、殘血或主動技能耗盡直接帶入下一個 Region。

此完整恢復發生在 Region 邊界，不代表每場一般戰鬥、Mini Boss 或事件戰鬥結束後都會自動完全回滿。
...
若該戰鬥同時完成一個 Region，則在 Region 結算後另外執行「Region 間完整恢復」，再進入下一個 Region。
