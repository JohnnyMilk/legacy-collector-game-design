...
- 敵人：4 名 Tier 1 一般敵人：荒路獵手、斷刃兵、灰燼灰燼術士、石鎧衛。
- 地圖：8×8。
...
- 玩家：4 名 Tier 1 主角。
- 敵人：12 名 Tier 1 一般敵人；荒路獵手、斷刃兵、灰燼灰燼術士、石鎧衛各 3 名。
- 地圖：8×8；8 個障礙物。
...
- 正式結論：**Tier 2 ×12＝Tier 2 玩家隊伍的「極限遭遇基準」**。4 名 Tier 2 玩家仍有通關可能，但已經非常勉強，代表這是目前確認的實戰承壓極限；不應作為 Region 2 一般標準遭遇。

## Boss Test

### Boss 2A｜Region 2 Mini Boss：森脈雙子｜完成
...
- 測試結果：完成，未發現問題。
- 正式結論：森脈雙子的名稱、個體數值、交叉破防技能、互補 AI、四系不重複的 Tier 2 玩家配置，以及右上角 3×3 障礙配置正式確認。

### Boss 2B｜Region 2 Boss：四印祭主｜實作中

- 玩家：四名固定主角，每次載入各抽取戰士、敏捷、魔法、支援系的一個隨機 Tier 2 職業，再隨機分配；職業系別不重複。
- 敵方：魔法系 Boss「四印祭主」+ 力量／防禦／魔防／魔攻圖騰各一個，不配置其他護衛。
- 圖騰：每個 HP 100、DEF 10、MDEF 10；不移動、不攻擊、不取得行動回合。存活時分別使 Boss 對應能力 +20%。
- 重建：圖騰被破壞後，Boss 下一次行動禁止重建，必須先以缺少該加成的狀態行動一次；再下一次行動才優先使用「圖騰再塑」，從缺少種類中隨機重建一個圖騰。一次一個、消耗整次行動，Charge 6。
- AI：持續鎖定同一名玩家直到該目標戰鬥不能；與目標距離大於 2 格時主動靠近，移動不得增加距離，不主動撤退或反覆橫向切換追擊目標。
- 地圖：8×8、無額外障礙物；Boss 固定於上方中央，四個圖騰每次載入隨機散落、彼此不相鄰，且不與玩家或 Boss 的初始位置相鄰。
- 狀態：測試頁已建立，待實機 Combat Test；名稱、數值、圖騰效果與技能暫維持 testing。
