...
### Demo 1｜Tier 1 極限測試：4 vs 12｜實作中
...
- 敗北：四名玩家全滅。
- 無 `forcePartyWipe`。
- 本階段目的：在完全不引入 Tier 2 敵人的前提下，測試四名 Tier 1 主角面對三倍數量 Tier 1 敵人的實際承壓極限。
- 不設定 Easy / Normal / Hard 等人工難度標籤；實際遭遇強度由玩家 Tier、敵方 Tier、敵人數量、兵種與地圖配置共同形成。
- Tier 1 主動技能、Charge 與被動技能：戰士「猛擊／堅守」、斥候「疾行／游擊」、法師「魔力彈／凝神」、牧師「治癒／恩典」。
- Party Composition、技能執行與技能 HUD 均屬共用 Combat Runtime，不得在 Demo 1 專屬程式內另做一套。
