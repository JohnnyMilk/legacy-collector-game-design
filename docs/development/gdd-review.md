...
Tier 3 外圈目前有 16 個概念節點；目前已完成正式設計：**軍團守衛、血煉騎士、戰將、獵刃武者、斷罪士、夜獵手、幻行師**。戰士系 4 個 Tier 3 已全部完成；敏捷系已有 3 個正式完成。**影刃術士**的職業名稱、Inspiration Links、Tier 3 專屬主／被動與 Mastery 已確認，僅一般攻擊尚待定義，因此目前維持 draft；其餘名稱與定位仍 provisional，必須依 Inspiration Links 的既有 Tier 2 技能包逐一重新設計。
...
- 定位：物魔交替／機動連段／連鎖爆發。
- 一般攻擊：**尚待定義**；一般攻擊不繼承，因此在確認前不擅自沿用任何 Tier 1 / Tier 2 一般攻擊。
...
- 完成條件：只剩一般攻擊名稱／型態需要確認；確認後即可將整職由 draft 升為 confirmed。
