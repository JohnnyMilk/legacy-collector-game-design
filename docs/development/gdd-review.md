...
Tier 3 外圈目前有 16 個概念節點；**軍團守衛、血煉騎士與戰將已完成正式設計**，其餘名稱與定位仍 provisional，必須依 Inspiration Links 的既有 Tier 2 技能包逐一重新設計。
...
- 一般攻擊「血刃」：只攻擊正前方 1 格的單一敵人，造成物理普通攻擊傷害，沒有額外效果。
...
- 定位：近身群戰／敵眾增傷／友軍成陣。
- 一般攻擊「橫掃」：以戰將面向為基準，同時攻擊左前方、正前方、右前方共 3 格；有效格內所有敵方單位各受到 1 次物理普通攻擊傷害。
- 被動「萬夫莫敵」：周圍 8 格內每存在 1 個敵方單位，造成傷害 +15%，依目前站位即時計算。
- 被動「眾志成城」：周圍 8 格內每存在 1 個友方單位，自身防禦 +15%，依目前站位即時計算。
- 戰將**不新增 Tier 3 專屬主動技能**，但仍完整保留繼承而來的合法主動技能。
- Mastery：周圍 8 格內至少 3 名敵人時成功造成傷害 20 次；周圍 8 格內至少 2 名友方時實際承受敵方造成的傷害 20 次。
